...
```
MetaScheduler.tick():
  1. MotivationEngine.computeDrives()  ← 5通道并行竞争 (BotContext + WorldContext + 环境)
  2. MotivationEngine.select()         ← 玻尔兹曼选择 + 交叉抑制
  3. labelProblem(perspective)         ← 贴标签 (现在是哪种问题)
  4. getFlowAdjustment(bot, state)     ← 升降级 (BotContext + MetaState)
  5. dispatch(label, flow)             ← 分派到对应执行层 (LLM门控)
```
